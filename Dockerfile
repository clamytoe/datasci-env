...
RUN /opt/conda/bin/conda update -n base -c defaults conda -y && \
    /opt/conda/bin/conda config --add channels conda-forge pytorch && \
    ...
    /opt/conda/bin/conda install pyyaml h5py keras tensorflow pydot graphviz nltk -y && \
    /opt/conda/bin/conda install pyppeteer imutils bs4 html5lib autoviz flask -y && \
    /opt/conda/bin/conda install fastai twine datasets evaluate transformers -y && \
    /opt/conda/bin/conda install pytorch torchvision torchaudio -y && \
